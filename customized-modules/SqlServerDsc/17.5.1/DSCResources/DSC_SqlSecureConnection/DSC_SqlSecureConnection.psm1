...
function Set-TargetResource
{
    ...
    # [YVAND] This is a workaround to be able to use the subject instead of the thumbprint in the DSC configuration
    $certThumbprint = Get-ChildItem -Path cert:\LocalMachine\My | Where-Object -FilterScript { $PSItem.Subject -eq $Thumbprint } | Select-Object -Expand Thumbprint
    if ($Ensure -eq 'Present')
    {
        if ($ForceEncryption -ne $encryptionState.ForceEncryption -or $certThumbprint -ne $encryptionState.Thumbprint)
        {
            Write-Verbose -Message (
                $script:localizedData.SetEncryptionSetting -f $InstanceName, $certThumbprint, $ForceEncryption
            )

            Set-EncryptedConnectionSetting -InstanceName $InstanceName -Thumbprint $certThumbprint -ForceEncryption $ForceEncryption
        }

        if ((Test-CertificatePermission -Thumbprint $Thumbprint -ServiceAccount $ServiceAccount) -eq $false)
        {
            Write-Verbose -Message (
                $script:localizedData.SetCertificatePermission -f $certThumbprint, $ServiceAccount
            )

            Set-CertificatePermission -Thumbprint $certThumbprint -ServiceAccount $ServiceAccount
        }
    }
    else
    {
...
function Get-CertificateAcl
{
    ...
    $cert = Get-ChildItem -Path cert:\LocalMachine\My | Where-Object -FilterScript { $PSItem.Subject -eq $Thumbprint }
