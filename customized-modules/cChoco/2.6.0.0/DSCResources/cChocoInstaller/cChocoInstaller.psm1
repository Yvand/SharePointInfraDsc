...
function Get-FileDownload {
	...
	# YVAND: Add a retry mechanism due to frequent network errors
    $count = 0
    $success = $false
    do {
        ...
        try {
            $count++
			Write-Verbose -Message ("Downloading '{0}' to '{1}'..." -f $url, $file)
            $downloader.DownloadFile($url, $file)
            $success = $true
        }
        catch {
            Write-Verbose -Message ("Download of '{0}' failed on attempt {1} with this error: {2}" -f $url, $count, $_.Exception.Message)
            ...
            if ($count -gt 10) {
				...
				$success = $false
				Write-Verbose -Message ("Attempt {1} to download again '{0}'" -f $url, $count + 1)
			}
